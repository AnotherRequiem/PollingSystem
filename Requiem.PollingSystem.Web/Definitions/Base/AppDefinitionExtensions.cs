...
namespace Requiem.PollingSystem.Web.Definitions.Base
{
    ...
    {
        public static void AddDefinitions(this IServiceCollection source,
            WebApplicationBuilder builder, params Type[] entryPointAssembly)
        {
            var definitions = new List<IAppDefinition>();

            foreach (var entryPoint in entryPointAssembly)
            {
        ...
        {
            var definitions = source.Services
                .GetRequiredService<IReadOnlyCollection<IAppDefinition>>();
            var environment = source.Services.GetService<IWebHostEnvironment>();
